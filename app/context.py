...
class CallContext:
    ...
    carc_codes: list[str]
    cpt_codes: list[str]
    member_id: str          # sent to payer IVR only, not LLM
    provider_npi: str
    ...
    async with driver.session(database=settings.neo4j_database) as s:
        result = await s.run(
            ...
            RETURN {
                ...
                carc_codes:   d.carcCodes,
                cpt_codes:    cpts,
                member_id:    coalesce(pt.memberId, pt.insuranceId, ''),
                provider_npi: coalesce(clm.renderingNpi, ''),
